...
def create_reader(data: list[dict[str, Any]]) -> AbstractIterReader:
    class TestReader(AbstractIterReader):
        def read(self):
            return data
...
def reader_with_error():
    class ErrorReader(AbstractIterReader):
        def read(self):
            x: list[dict[str, Any]] = [
...
def test_init(reader: AbstractIterReader):
    IterValidator(schema=Schema, reader=reader)


def test_validate(reader: AbstractIterReader):
    validator = IterValidator(schema=Schema, reader=reader)
    data, _ = validator.validate_all()
    assert list(data) == [
        {"a": 1, "b": "hello", "c": 2.5},
        {"a": 2, "b": "world", "c": 3.0},
    ]
...
def test_validate_with_error(reader_with_error: AbstractIterReader):
    validator = IterValidator(schema=Schema, reader=reader_with_error)
